...
toolbox-export {--list|--purge|--update}
...
--update/-u::		
    updates exported app starters and binaries to use the current toolbox
...
With the option `--update` or `-u` it is possible to move all starters and exported binaries
from old toolboxes to the latest one with one command. 
This is especially useful after a distro upgrade
when the standard container like `fedora-toolbox-42` is rebuilt (e.g. to `fedora-toolbox-43`).
The update options supports also a dry-run option "-n". 

	toolbox enter 
  	sudo dnf install toolbox-export
	toolbox-export -n --update # dry-run
	toolbox-export --update
